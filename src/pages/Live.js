...
function Live() {
    ...
    function updateCode(e, value) {
        ...
        db.ref("/joins/" + activeroom + "/code/code").set(value)
            .catch(error => {
    ...
    const logOut = () => {
        ...
        auth.signOut().then(() => {
            db.ref('/joins/' + activeroom + '/contributors/' + username).child('status').set('offline');
    ...
    useEffect(function () {
        ...
        if (activeroom) {
            ...
            // Online status
            db.ref("/joins/" + activeroom + "/contributors/" + localStorage.getItem('username')).child('status').set('online');
        }

        return function () {
            db.ref("/joins/" + activeroom + "/code/").child('code').off();
        }
    }, [activeroom]);
        ...
        socket.on("requestaccess", (data) => {
            db.ref("/joins/" + data.room).once("value", (snap) => {
                if (snap.val().initiator === localStorage.getItem("username")) {
                    let access = window.confirm(
                        data.username + " wants to collaborate to " + data.room
                    );
                    data.access = access;
